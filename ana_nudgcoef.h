...
!
!! svn $Id: ana_nudgcoef.h 709 2014-01-23 20:09:38Z arango $
!!================================================= Hernan G. Arango ===
...
!
     ...
     &                   + (width - (GRID(ng)%yr(i,j)-nudgwidth))/width
            ...
            END IF
#endif
! assign to nudging arrays
#if defined M2CLM_NUDGING
            CLIMA(ng)%M2nudgcof(i,j) = MIN(cff1*M2nudg(ng),M2nudg(ng))
#endif
#if defined M3CLM_NUDGING
            CLIMA(ng)%M3nudgcof(i,j) = MIN(cff1*M3nudg(ng),M3nudg(ng))
#endif
#if defined TCLM_NUDGING
            DO itrc=1,NT(ng)
               CLIMA(ng)%Tnudgcof(i,j,itrc) = MIN(cff1*Tnudg(itrc,ng),  &
     &                                          Tnudg(itrc,ng))
            END DO
#endif
